  ...
  return <main className="mx-auto max-w-6xl px-5 py-10 sm:px-8 sm:py-14">
    ...
    <section className="mt-10 overflow-hidden rounded-xl border border-[#292d29]">
      ...
      {query.data?.map((session) => {
        const canResume = session.status === "created" || session.status === "active";
        return <Link key={session.id} to={canResume ? `/app/interviews/${session.id}/live` : `/app/interviews/${session.id}`} className="group flex items-center justify-between border-b border-[#292d29] bg-[#0d0f0d] p-5 last:border-0 hover:bg-[#121512]"><div><h2 className="font-medium">{labels[session.type]} interview</h2><p className="mt-1 text-xs text-[#90968e]">{session.targetRole} · {new Date(session.createdAt).toLocaleDateString()}</p></div><div className="flex items-center gap-4"><span className={`text-xs ${session.status === "report_ready" ? "text-[#caff3d]" : session.status === "failed" ? "text-red-300" : "text-[#90968e]"}`}>{session.status === "report_ready" ? `${session.report?.overallScore ?? "—"}/100` : canResume ? "resume" : session.status.replace("_", " ")}</span><ArrowRight className="size-4 text-[#666c64] transition-transform group-hover:translate-x-1" /></div></Link>;
      })}
    </section>
  </main>;
}
